...
/* Build the assistant content array for messages history.
 * Prefer the full content returned by provider to preserve reasoning/thinking blocks. */
static cJSON *build_assistant_content(const llm_response_t *resp)
{
    if (resp->assistant_content && cJSON_IsArray(resp->assistant_content)) {
        cJSON *dup = cJSON_Duplicate(resp->assistant_content, 1);
        if (dup && cJSON_IsArray(dup)) return dup;
        cJSON_Delete(dup);
    }

    /* Fallback for providers that do not return structured content blocks. */
